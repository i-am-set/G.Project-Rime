...
func updateLights():
	...
	match cloudType:
		"Static":
			sun.light_energy = clamp(skyPreset.sunLightIntensity.sample(sunPosition),0.0,3.0)
			moon.light_energy = clamp(skyPreset.moonLightIntensity.sample(sunPosition),0.0,3.0)
		"2D Dynamic":
