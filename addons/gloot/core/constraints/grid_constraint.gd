...
func set_item_rotation(item: InventoryItem, rotated: bool, is_held: bool) -> bool:
	...
	if rotated:
		if is_held:
			item.set_held_property(KEY_ROTATED, true)
		else:
			item.set_property(KEY_ROTATED, true)
	else:
		if is_held:
			item.clear_held_property(KEY_ROTATED)
		else:
			item.clear_property(KEY_ROTATED)

	return true

func rotate_item(item: InventoryItem, is_held: bool) -> bool:
	return set_item_rotation(item, !is_item_rotated(item), is_held)
